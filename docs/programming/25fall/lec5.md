...
![](lec5.assets/Arpanet_logical_map,_march_1977.png)
...
![](lec5.assets/localhost-meme.avif)
...
- 一般情况下，不同的软件使用不同的端口号
    - 通过端口号可以区分不同的服务，例如 HTTP 的分配端口是 TCP 80
        - 即 <http://10.214.96.145> 就是 <http://10.214.96.145:80>
        - 否则需要写明端口号，如 <http://10.214.96.145:39200>
        - 稍后在讲 HTTP 时会加深理解
...
![](lec5.assets/URI_syntax_diagram.svg)

/// caption
URL Syntax Diagram

来源: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:URI_syntax_diagram.svg)
///
...
- [HTTP Overview - MDN Web Docs](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Overview)
...
- HTTPS：HTTP over TLS
    ...
    - [The Illustrated TLS 1.2 Connection: Every Byte Explained](https://tls12.xargs.org) ([中文版](https://cangsdarm.github.io/illustrate/tls12))
...
!!! info inline end ""
    ...
    ![](lec5.assets/protocol-stack.svg){ align=right }

    /// caption
    HTTP/1.1 vs. HTTP/2 vs. HTTP/3 Protocol Stack

    来源: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:HTTP-1.1_vs._HTTP-2_vs._HTTP-3_Protocol_Stack.svg)
    ///
...
- HTTP/3 基于 QUIC 协议
    ...
    - 通过其他手段克服 UDP 的不可靠性
    - [HTTP/3：过去，现在，还有未来](https://blog.cloudflare.com/zh-cn/http3-the-past-present-and-future/)
...
!!! info inline end ""
    ...
    ![](lec5.assets/WebAssembly_Logo.svg){ align=right }

    /// caption
    WebAssembly Logo

    来源: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:WebAssembly_Logo.svg)
    ///
