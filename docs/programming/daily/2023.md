...
??? note "Answer"
    ...
    `sizeof(A)` is 14, and `sizeof(B)` is 8 (on a 64-bit modern system).
    ...
    The size of `A` is 14, because the size of `char[10]` is 10, and the size of `int` is 4. The size of `B` is 8, because `B` is a pointer and the size of pointer is 8. The size of type to which `B` points does not affect the size of `B`.
